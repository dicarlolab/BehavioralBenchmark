...
def empirical_consistency(M, n_boots):
    ...
    def get_human_data_densely_sampled():
        ...
        canonical_order = dataset.meta['filename'][ImageSet1_inds]
    ...
    def empirical_consistency_to_humans(human_reps, human_matrix, number_of_iterations):

        # It computes the individual human to pool empirical consistency

        if len(human_matrix.shape) == 1:
            consistencies = [metric(human_matrix, human_reps.mean(1))]
        else:
            consistencies = np.zeros(number_of_iterations)
            for nb in range(number_of_iterations):
                spH0, spH1 = split_half_reps(human_reps)
                spM0, spM1 = split_half_reps(human_matrix)
